...
// J-type: imm[20|10:1|11|19:12] -> sext(20)
#define immJ() do { \
...
// B-type: imm[12|10:5|4:1|11] -> sext(12)
#define immB() do { \
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  // Shift Instructions (注意 src1 是被移位数, imm 是移位量)
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I,
    ...
    R(rd) = s->pc + 4;
    s->dnpc = target;
    // jalr 用于函数返回时，rd=x0 且 rs1=x1(ra)
    // 提取 rs1 字段进行判断
    int rs1_reg = BITS(s->isa.inst, 19, 15);
    if (rd == 0 && rs1_reg == 1) {
      IFDEF(CONFIG_FTRACE, ftrace_ret(s->pc));
    } else {
      IFDEF(CONFIG_FTRACE, ftrace_call(s->pc, target));
    }
  );
